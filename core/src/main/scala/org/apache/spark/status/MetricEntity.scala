...
import org.apache.spark.util.{AccumulatorContext, ConfigurationUtil}
 ...
 */
private[spark] abstract class LiveMetricEntity {

  var lastWriteTime = -1L
  var kafkaSwitch = "spark_kafka_sink_switch"
  def report(kafkaSink: KafkaSink, eventName: String, eventType: String): Unit = {
    ...
    if (getValue(kafkaSwitch).getOrElse("true").toBoolean) {
      kafkaSink.report(KafkaSink.METRIC_TOPIC,
        new SimpleWrapEvent(eventType, eventName, doUpdate()))
    }
  }
...
private class LiveMetricJob(
    ...
    numTasks: Int) extends LiveMetricEntity {
  ...
  override protected def doUpdate(): Any = {
    new JobMetricData(
      ...
      appId,
      jobId,
      jobGroup.getOrElse("null"),
      null, // description is always None?
      submissionTime.getOrElse(new Date(-1)),
      completionTime.getOrElse(new Date(-1)),
      complete,
      status.name(),
      stageIds,
      numTasks,
...
private class LiveMetricTask(
    ...
    lastUpdateTime: Option[Long]) extends LiveMetricEntity {
  ...
  override protected def doUpdate(): Any = {
    ...
    new TaskMetricData(
      ...
      info.index,
      info.attemptNumber,
      startTime.getOrElse(new Date(-1)),
      endTime,
      duration,
...
private class LiveMetricExecutor(val executorId: String, _addTime: Long) extends LiveMetricEntity {
...
private class LiveMetricExecutorStageSummary(
    stageId: Int,
    attemptId: Int,
    executorId: String) extends LiveMetricEntity {
...
private class LiveMetricStage(appId: String) extends LiveMetricEntity {

  import LiveEntityHelpers._

  var id: Option[String] = None
  var jobs = Seq[LiveMetricJob]()
  var jobIds = Set[Int]()
  ...
  val executorSummaries = new HashMap[String, LiveMetricExecutorStageSummary]()
  ...
  def executorSummary(executorId: String): LiveMetricExecutorStageSummary = {
    executorSummaries.getOrElseUpdate(executorId,
      new LiveMetricExecutorStageSummary(info.stageId, info.attemptNumber, executorId))
  }


  def toData(): StageMetricData = {
    new StageMetricData(
      id.getOrElse("null"),
      appId,
      jobIds,
...
private class LiveMetricRDDPartition(val blockName: String) {
...
private class LiveMetricRDDDistribution(exec: LiveExecutor) {
...
private class LiveMetricRDD(val info: RDDInfo) extends LiveMetricEntity {
...
private class SchedulerPoolMetric(name: String) extends LiveMetricEntity {
...
private object LiveMetricEntityHelpers {
