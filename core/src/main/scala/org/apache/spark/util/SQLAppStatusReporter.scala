...
private[spark] class SQLAppStatusReporter(conf: SparkConf) {
...
private[spark] object SQLAppStatusReporter {
  ...
  def createAppStatusReporter(conf: SparkConf): SQLAppStatusReporter = {
    new SQLAppStatusReporter(conf)
  }
}
