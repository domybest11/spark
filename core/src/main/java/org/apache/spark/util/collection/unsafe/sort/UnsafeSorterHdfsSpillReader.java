...
public final class UnsafeSorterHdfsSpillReader extends UnsafeSorterIterator implements Closeable {
    ...
    public UnsafeSorterHdfsSpillReader(
        ...
        try {
            if (readAheadEnabled) {
                this.in = new ReadAheadInputStream(serializerManager.wrapStream(blockId, fsDataInputStream),
                        bufferSizeBytes);
            } else {
                this.in = serializerManager.wrapStream(blockId, fsDataInputStream);
            }
            this.din = new DataInputStream(this.in);
            numRecords = numRecordsRemaining = din.readInt();
        } catch (IOException e) {
            Closeables.close(fsDataInputStream, /* swallowIOException = */ true);
            throw e;
        }
    }
