...
class ThriftServerSqlAppStatusStore(
  ...
  def assembleExecutionInfo(executionInfo: ExecutionInfo): Option[ApplicationSQLExecutionData] = {
    ...
    executionInfo.jobId.foreach(jobid => {
      val jobData = job(jobid.toInt)
      if (jobData.isDefined) {
        if (executionInfo.state == ExecutionState.FINISHED
    ...
    executionInfo.jobs = listSQLJobData
    executionInfo.executionId = if (executionsList.nonEmpty) {
     val executionIds = executionsList.filter(execution =>
        execution.jobs.keySet.intersect(executionInfo.jobId.map(_.toInt).toSet).nonEmpty)
        .map(_.executionId)
      if (executionIds.nonEmpty) executionIds.head else -1
    } else {
      -1
    }
    applicationSQLExecutionData.sqlExecutionData = Some(executionInfo)
  ...
  def stageAttempt(stageId: Int, stageAttemptId: Int, details: Boolean = false): StageData = {
