...
import org.apache.spark.sql.hive.thriftserver.ui.{ExecutionInfo, HiveThriftServer2Listener}


object ThriftServerAppStatusScheduler{
  val POISON_PILL = new ExecutionInfo("", "", 0, "")
}
...
class ThriftServerAppStatusScheduler extends Logging{
  ...
  val _executionInfoQueue =
    new LinkedBlockingQueue[ExecutionInfo](EXECUTION_INFO_QUEUE_CAPACITY)
  val sendOnceMetricKafkaThread = new Thread("sink-once-metric-task") {
    setDaemon(true)
    override def run(): Unit = {
      var next: ExecutionInfo = _executionInfoQueue.take()
  ...
  def start(appStatusStoreV1: ThriftServerSqlAppStatusStore): Unit = {
   ...
   def sendPeriodlyKafka(): Unit = {
    ...
    var sqlExecutionList: Seq[ExecutionInfo] = listener.getExecutionList
   ...
   def sendOnceKafka(executionInfo: ExecutionInfo): Unit = {
