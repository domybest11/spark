...
package org.apache.spark.sql.catalyst.expressions.zorder;
...
public class Morton64 implements Serializable {
    ...
    public Morton64(long dimensions, long bits) {
        if (dimensions <= 0 || bits <= 0 || dimensions * bits > 64 || (dimensions == 1 && bits == 63)) {
            throw new InterleaveException(String.format("can't make morton64 with %d dimensions and %d bits", dimensions, bits));
        }
    ...
    private void dimensionsCheck(long dimensions) {
        if (this.dimensions != dimensions) {
            throw new InterleaveException(String.format("morton64 with %d dimensions received %d values", this.dimensions, dimensions));
        }
    }

    private void valueCheck(long value) {
        if (value < 0 || value >= (1L << this.bits)) {
            throw new InterleaveException(String.format("morton64 with %d bits per dimension received %d to pack", this.bits, value));
        }
    }

    private long shiftSign(long value) {
        if (value >= (1L << (bits - 1)) || value <= -(1L << (bits - 1))) {
            throw new InterleaveException(String.format("morton64 with %d bits per dimension received signed %d to pack", this.bits, value));
        }
