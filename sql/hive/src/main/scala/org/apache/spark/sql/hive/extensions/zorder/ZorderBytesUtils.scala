...
package org.apache.spark.sql.hive.extensions.zorder
...
object ZorderBytesUtils {
  ...
  def toLong(a: Any): Long = {
    a match {
      ...
      case other: Any =>
        throw new SparkException("Unsupported z-order type: " + other.getClass)
    }
  }
  ...
  def defaultValue(dataType: DataType): Any = {
    dataType match {
      ...
      case other: Any =>
        throw new SparkException(s"Unsupported z-order type: ${other.catalogString}")
    }
  }
}
